create table if not exists public.user_items (
  ...
  year text,
  overview text,
  status text not null default 'watchlist' check (status in ('watchlist', 'watching', 'watched', 'stopped')),
...
drop policy if exists "Anyone can read active Streamory alerts" on public.alerts;

create policy "Anyone can read active Streamory alerts"
...
alter table public.user_items
  drop constraint if exists user_items_status_check;
alter table public.user_items
  add constraint user_items_status_check
  check (status in ('watchlist', 'watching', 'watched', 'stopped'));
...
drop function if exists public.list_streamory_friends();
...
create or replace function public.list_streamory_friend_watchlist(target_user_id uuid)
returns table (
  id uuid,
  user_id uuid,
  tvdb_id text,
...
as $$
  ...
  where auth.uid() is not null
    and ui.user_id = target_user_id
    and ui.status = 'watchlist'
    and exists (
      select 1
      from public.friend_requests fr
...
create or replace function public.get_streamory_library(target_user_id uuid)
returns table (
  id uuid,
  user_id uuid,
  tvdb_id text,
...
as $$
  ...
  order by
    case ui.media_type when 'series' then 0 else 1 end,
    lower(ui.title);
$$;

grant execute on function public.get_streamory_library(uuid) to authenticated;

create or replace function public.get_streamory_episode_watches(target_user_id uuid, target_series_tvdb_id text)
returns table (
...
as $$
  ...
  where user_id = target_user_id
    and series_tvdb_id = target_series_tvdb_id
  order by season_number, episode_number;
$$;

grant execute on function public.get_streamory_episode_watches(uuid, text) to authenticated;

create or replace function public.delete_current_streamory_user(delete_password text)
returns void
language plpgsql
security definer
...
declare
  ...
  has_email_identity boolean := false;
  has_apple_identity boolean := false;
begin
  ...
  if stored_password is not null and stored_password <> '' and (has_email_identity or not has_apple_identity) then
    if delete_password is null or delete_password = '' or stored_password <> crypt(delete_password, stored_password) then
...
revoke all on function public.delete_current_streamory_user(text) from public;
grant execute on function public.delete_current_streamory_user(text) to authenticated;

create or replace function public.delete_current_streamory_user()
returns void
language sql
security definer
set search_path = public, auth, extensions
as $$
  select public.delete_current_streamory_user(null::text);
$$;

revoke all on function public.delete_current_streamory_user() from public;
grant execute on function public.delete_current_streamory_user() to authenticated;
